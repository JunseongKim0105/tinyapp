...
app.post('/urls', (req, res) => {
  if (req.session.user_id) {
    const longURL = req.body.longURL.startsWith('http://')
      ? req.body.longURL
    ...
    res.send(
      '<html><h2>Sorry only login user gets privileges of creating new short URL</h2></html>'
    );
  }
});
...
app.get('/u/:id', (req, res) => {
  ...
  if (!urlDatabase[id]) {
    res.send(
      '<html><h2>Sorry no relevent ID was found in our database. </h2></html>'
    );
    return;
  }
  const userId = req.session['user_id'];
...
app.post('/urls/:id/delete', (req, res) => {
  ...
  if (!urlDatabase[id]) {
    ...
    res.send(
      '<html><h2>Sorry you have no permission of changing the URL. </h2></html>'
    );
    return;
  }
...
app.get('/urls/:id', (req, res) => {
  ...
  if (!userId) {
    res.send(
      '<html><h2>Sorry only login user gets privileges of reviewing short URLs</h2></html>'
    );
    return;
...
app.put('/urls/:id', (req, res) => {
  ...
  const newURL = req.body.longURL.startsWith('http://')
    ? req.body.longURL
    : `http://${req.body.longURL}`;
